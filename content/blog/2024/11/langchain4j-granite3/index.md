...
That said, I took a closer look a the [license](https://github.com/meta-llama/llama/blob/main/LICENSE) of the codellama model used in the example.
At first, the license looks quite permissive and we need to acknowledge that it's really good from a commercial company to help with spreading LLMs to a large public.
...
Thinking a bit more, a true open source LLM should probably open up the training data and does whatever it takes to have a community being able to contribute ala [instructlab](https://instructlab.ai/). But that's a big subject far beyond the scope of this small blog post.
...
However, how could we ensure that the situation is not worst than before the migration.
For this, we would need to estimate possible regressions in distinct areas listed below.

## Accuracy

First one need to check that the model output is still accurate enough.
...
Therefore, the summaries comparison was done manually.
...
Now come the third points of our non regression analysis, the performance.
...
These are just a few experiments, yet we start to see what could be needed in a true production scenario where a model switch is needed:
 ...
 + A way to measure the determnism
